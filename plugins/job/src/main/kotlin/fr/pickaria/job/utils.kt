 ...
 */
internal fun getExperienceFromLevel(job: JobConfig, level: Int): Int =
	if (level >= 0) {
 ...
 */
internal fun getLevelFromExperience(job: JobConfig, experience: Double): Int {
	var level = 0
	var levelExperience = job.startExperience.toDouble()
	while ((levelExperience + level * job.multiplier) < experience && level < Config.maxLevel) {
 ...
 */
internal fun getAscentPoints(job: Job, config: JobConfig): Int =
	getLevelFromExperience(config, job.experience).let {
		if (it >= Config.ascent.startLevel) {
			(it - Config.ascent.startLevel) / Config.ascent.pointEvery * Config.ascent.pointAmount + 1
		} else {
			0
		}
	}